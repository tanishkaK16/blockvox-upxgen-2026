...
import { saveElectionData, type Candidate } from '@/lib/election-store';
import {
  buildMerkleTree,
  parseVoterFile,
  saveMerkleTree,
...
export default function AdminPage() {
  ...
  const handleCreateElection = async () => {
    ...
    addLog(`Creating election: "${electionName}"`);
    addLog('Submitting startElection() transaction...');
    ...
    if (liveMode && walletClient && publicClient && address) {
      try {
        ...
        addLog(`Confirmed in block ${receipt.blockNumber} (gas: ${receipt.gasUsed})`);
        addLog('Election is now ACTIVE ✓');
        setElectionStatus('active');
        // Notify others via storage event
      ...
      setTimeout(() => {
        ...
        addLog(`Transaction confirmed: ${fakeTx.slice(0, 18)}...`);
        addLog('Election is now ACTIVE ✓');
        setElectionStatus('active');
        // Notify others via storage event
        window.dispatchEvent(new Event('storage'));
