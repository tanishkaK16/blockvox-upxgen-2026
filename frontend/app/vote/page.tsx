...
export default function VotePage() {
  ...
  const handleCommit = useCallback(async () => {
    ...
    setSalt(newSalt);
    const commitment = computeCommitmentHash(selectedCandidate, newSalt);
    ...
    if (liveMode && walletClient && publicClient && address) {
      ...
      try {
        ...
        setLoadingStep('Awaiting wallet signature...');
        const txHash = await commitVoteOnChain(commitment, proofNodes as `0x${string}`[], walletClient, address);
  ...
  const handleReveal = useCallback(async () => {
    ...
    if (liveMode && walletClient && publicClient && address) {
      // ─── LIVE: Real on-chain reveal ───
      try {
        setLoadingStep('Preparing reveal payload...');
        await new Promise(r => setTimeout(r, 400));

        setLoadingStep('Sending revealVote(candidateId, salt)...');
      ...
      // ─── DEMO: Simulated reveal ───
      setLoadingStep('Preparing reveal payload...');
      setTimeout(() => setLoadingStep('Sending revealVote(candidateId, salt)...'), 800);
      setTimeout(() => setLoadingStep('Awaiting wallet signature...'), 1600);
      setTimeout(() => setLoadingStep('Broadcasting to Avalanche Fuji...'), 2400);
      setTimeout(() => setLoadingStep('Verifying commitment match on-chain...'), 3200);
  ...
  }, [selectedCandidate, salt, liveMode, walletClient, publicClient, address]);
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                  {candidates.map((c) => (
                    ...
                    >
                      ...
                      <span className="text-xs text-gray-500 ml-8 block mt-1 relative z-10">{c.party}</span>
                    </button>
                  ))}
                </div>
